  ...
  // ── Three-Act Structure ─────────────────────────────────────────────────
  {
    ...
    phases: [
      ...
      },
      {
        ...
        structuralRules: 'CONVERGENCE: The midpoint must create a cross-thread collision — at least 2 threads must causally intersect, not just escalate in parallel. Subplots must intersect the main thread at least twice. PAYOFF DENSITY: Medium — no more than 3 consecutive scenes of pure escalation without a drive beat (thread transition, irreversible consequence, or relationship fracture). SCENE FUNCTION VARIETY: Alternate between action scenes (confrontation, pursuit, gamble) and consequence scenes (adaptation, cost, reframing). No more than 2 of the same function in any arc. BEAT-TYPE BAN: No repeated "protagonist tries same approach and fails" — each failure must use a different mechanism and teach a different lesson.',
    ...
    phases: [
      {
        ...
        objective: 'Reintroduce the world after the gap between volumes. Show how characters have changed since last time. Establish the status quo of THIS volume — new year, new setting, new dynamics. Reactivate latent threads from previous volumes through small signals. Introduce the volume\'s unique hook — a mystery, a new character, an unusual event that will drive this particular story.',
        sceneAllocation: 7,
        constraints: 'Do not resolve overarching series threads. This phase re-establishes, not concludes.',
        structuralRules: 'SCENE FUNCTION VARIETY: Alternate between familiar (reintroduction, changed dynamics) and new (hook, unique element, fresh mystery). No more than 2 consecutive reintroduction scenes. Each reactivated latent thread must create a question, not just a reminder. PROTAGONIST GRAVITY: At least 5 of 7 scenes must feature the protagonist directly. PAYOFF DENSITY: Low — each scene plants 2-3 seeds. Maximum 1 thread transition (the hook activation).',
